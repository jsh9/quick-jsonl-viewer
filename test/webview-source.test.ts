...
test('line count errors persist through webview rerenders', async () => {
  ...
  assert.match(source, /function setLineCountText\(state, value, progress\) \{[\s\S]*?state === 'unavailable'[\s\S]*?lineCount\.textContent = 'Unavailable';/);
});
...
test('open viewers reload when their file changes', async () => {
  ...
  assert.match(source, /if \(fileReloadTimer\) \{[\s\S]*?clearTimeout\(fileReloadTimer\);[\s\S]*?\}/);
});
...
test('line count progress is posted and rendered', async () => {
  ...
  assert.match(source, /onProgress: \(progress\) => \{[\s\S]*?type: 'lineCountProgress',[\s\S]*?payload: progress/);
  assert.match(source, /if \(message\.type === 'lineCountProgress'\) \{[\s\S]*?data\.lineCountState = 'counting';[\s\S]*?data\.lineCountProgress = progress;/);
  assert.match(source, /if \(message\.type === 'lineCountProgress'\) \{[\s\S]*?full\.lineCountState = 'counting';[\s\S]*?full\.lineCountProgress = progress;/);
  assert.match(source, /function normalizeLineCountProgress\(payload\) \{/);
  assert.match(source, /lineCount\.textContent = progress \? 'Counting ' \+ formatPercent\(progress\.percent\) : 'Counting\.\.\.';/);
});

test('line counts are cached across settings-only reloads', async () => {
  const source = await readExtensionSource();
  const configurationReload = /vscode\.workspace\.onDidChangeConfiguration\(\(event\) => \{([\s\S]*?)\n      \}\)/.exec(source)?.[1] ?? '';
  ...
  assert.match(source, /interface FileSnapshot \{[\s\S]*?readonly size: number;[\s\S]*?readonly mtimeMs: number;/);
  assert.match(source, /let exactLineCountCache: ExactLineCountCache \| undefined;/);
  assert.match(source, /let exactLineCountRequest: ExactLineCountRequest \| undefined;/);
  assert.match(configurationReload, /safeLoad\(\);/);
  assert.doesNotMatch(configurationReload, /invalidateExactLineCount|abortExactLineCount|ensureExactLineCount/);
});
...
test('file snapshot changes invalidate exact line counts', async () => {
  ...
  assert.match(source, /function getFileSnapshot\(stats: Pick<nodeFs\.Stats, 'size' \| 'mtimeMs'>\): FileSnapshot/);
  assert.match(source, /function isSameFileSnapshot\(left: FileSnapshot, right: FileSnapshot\): boolean \{[\s\S]*?left\.size === right\.size && left\.mtimeMs === right\.mtimeMs/);
  assert.match(source, /const noteFileSnapshot = \(snapshot: FileSnapshot\): void => \{[\s\S]*?invalidateExactLineCount\(\);[\s\S]*?currentFileSnapshot = snapshot;/);
  assert.match(source, /const snapshot = getFileSnapshot\(stats\);[\s\S]*?exactLineCounts\.noteFileSnapshot\(snapshot\);/);
});
...
test('exact line counting reuses cache and in-flight requests', async () => {
  ...
  assert.match(source, /const getCachedLineCount = \(snapshot: FileSnapshot\): number \| undefined =>[\s\S]*?isSameFileSnapshot\(exactLineCountCache\.snapshot, snapshot\)/);
  assert.match(source, /const ensureExactLineCount = \(snapshot: FileSnapshot\): void => \{[\s\S]*?if \(getCachedLineCount\(snapshot\) !== undefined\) \{[\s\S]*?return;[\s\S]*?if \(exactLineCountRequest && isSameFileSnapshot\(exactLineCountRequest\.snapshot, snapshot\)\) \{[\s\S]*?return;/);
  assert.match(source, /lineCount: exactLineCounts\.getCachedLineCount\(snapshot\) \?\? null,/);
  assert.match(source, /if \(index\.isComplete\) \{[\s\S]*?exactLineCounts\.setCachedLineCount\(snapshot, index\.indexedLineCount\);/);
...
test('virtual scrolling uses capped physical spacer and logical offsets', async () => {
  ...
  assert.match(source, /function pruneMeasuredRowHeights\(start, count\) \{[\s\S]*?measuredRowHeights\.size <= MAX_MEASURED_ROW_HEIGHTS/);
